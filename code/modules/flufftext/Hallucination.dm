	...
	switch(battle_type)
		...
		if("disabler")
			...
			for(var/i in 1 to rand(5, 10))
				...
				if(hits >= 3 && prob(70))
					...
					break
		if("esword")
			target.playsound_local(source, 'sound/blank.ogg',15, 1)
		...
		if("bomb") // Tick Tock
			for(var/i in 1 to rand(3, 11))
				target.playsound_local(source, 'sound/blank.ogg', 25, 0)
				sleep(15)
	qdel(src)
...
/datum/hallucination/stationmessage/New(mob/living/carbon/C, forced = TRUE, message)
	...
	switch(message)
		...
		if("ratvar")
			target.playsound_local(target, 'sound/blank.ogg', 50, FALSE, pressure_affected = FALSE)
			target.playsound_local(target, 'sound/blank.ogg', 50, FALSE, pressure_affected = FALSE)
			sleep(27)
			target.playsound_local(target, 'sound/blank.ogg', 50, FALSE, pressure_affected = FALSE)
		if("shuttle dock")
		...
		if("malf ai") //AI is doomsdaying!
			...
			to_chat(target, "<br><br><span class='alert'>Hostile runtimes detected in all station systems, please deactivate my AI to prevent possible damage to its morality core.</span><br><br>")
			SEND_SOUND(target, 'sound/blank.ogg')
...
/datum/hallucination/death/New(mob/living/carbon/C, forced = TRUE)
	...
	if(prob(50))
		...
		if(fakemob)
			sleep(rand(20, 50))
			to_chat(target, "<span class='deadsay'><b>DEAD: [fakemob.name]</b> dice, \"[pick("rip","why did i just drop dead?","hey [target.first_name()]","git gud","you too?","is the AI rogue?",\
