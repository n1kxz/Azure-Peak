...
/mob/living/simple_animal/revenant/Login()
	..()
	to_chat(src, span_deadsay("<span class='big bold'>I are a revenant.</span>"))
...
/mob/living/simple_animal/revenant/Life()
	...
	if(unreveal_time && world.time >= unreveal_time)
		...
		incorporeal_move = INCORPOREAL_MOVE_JAUNT
		invisibility = INVISIBILITY_REVENANT
		to_chat(src, span_revenboldnotice("I are once more concealed."))
...
/mob/living/simple_animal/revenant/attackby(obj/item/W, mob/living/user, params)
	...
	if(istype(W, /obj/item/nullrod))
		visible_message(span_warning("[src] violently flinches!"), \
						span_revendanger("As \the [W] passes through you, you feel my essence draining away!"))
...
/mob/living/simple_animal/revenant/death()
	if(!revealed || stasis) //Revenants cannot die if they aren't revealed //or are already dead
		return 0
	stasis = TRUE
	to_chat(src, span_revendanger("NO! No... it's too late, you can feel my essence [pick("breaking apart", "drifting away")]..."))
	notransform = TRUE
	revealed = TRUE
	invisibility = 0
