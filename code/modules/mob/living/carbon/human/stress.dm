...
/mob/living/carbon/update_stress()
	...
	if(new_stress != oldstress)
		...
		if(diff_abs > 1)
			if(ascending)
				to_chat(src, span_smallred("Gano estres."))
			else
				to_chat(src, span_smallgreen("Gano paz."))
	...
	if(old_threshold != new_threshold)
		...
		switch(new_threshold)
			if(STRESS_THRESHOLD_NICE)
				to_chat(src, span_green("Me siento genial!"))
				apply_status_effect(/datum/status_effect/mood/vgood)
			if(STRESS_THRESHOLD_GOOD)
				if(ascending)
					to_chat(src, span_info("Ya no me siento tan bien"))
				else
					to_chat(src, span_green("Me siento bien."))
				apply_status_effect(/datum/status_effect/mood/good)
			if(STRESS_THRESHOLD_NEUTRAL)
				if(ascending)
					to_chat(src, span_info("Ya no me siento bien."))
				else
					to_chat(src, span_info("Ya no me siento estresado."))
			if(STRESS_THRESHOLD_STRESSED)
				if(ascending)
					to_chat(src, span_red("Me estoy estresando..."))
				else
					to_chat(src, span_red("Estoy algo menos estresado."))
				apply_status_effect(/datum/status_effect/mood/bad)
			if(STRESS_THRESHOLD_STRESSED_BAD)
				if(ascending)
					to_chat(src, span_boldred("Estoy llegando al limite.."))
				else
					to_chat(src, span_boldred("Ya no estoy perdiendo la cabeza..."))
				apply_status_effect(/datum/status_effect/mood/vbad)
			if(STRESS_THRESHOLD_FREAKING_OUT)
				to_chat(src, span_boldred("ESTOY PERDIENDO LA CABEZA!!!"))
				apply_status_effect(/datum/status_effect/mood/vbad)
